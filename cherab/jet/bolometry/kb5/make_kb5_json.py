...
foils = np.loadtxt('kb5h_foils.csv', delimiter=',')
slits = np.loadtxt('kb5h_slits.csv', delimiter=',')


kb5horizontal = BolometerCamera(name="KB5H")

slit_objects = {}
for i in range(4):
    slit_data = slits[i]
    slit_id = 'KB5H_SLIT_{}'.format(str(int(slit_data[0])))
    p1 = Point3D(slit_data[1], slit_data[2], slit_data[3])
    p2 = Point3D(slit_data[4], slit_data[5], slit_data[6])
    p3 = Point3D(slit_data[7], slit_data[8], slit_data[9])
    p4 = Point3D(slit_data[10], slit_data[11], slit_data[12])
    ...
    dy = p2.distance_to(p3)
    centre_point = Point3D((p1.x + p2.x + p3.x + p4.x)/4, (p1.y + p2.y + p3.y + p4.y)/4, (p1.z + p2.z + p3.z + p4.z)/4)
    slit_objects[slit_id] = BolometerSlit(slit_id, centre_point, basis_x, dx, basis_y, dy)

for i in range(24):

    foil_data = foils[i]
    foil_id = 'KB5H_CH{}'.format(str(int(foil_data[0])))
    slit_id = 'KB5H_SLIT_{}'.format(str(int(foil_data[1])))
    ...
    # Shift backwards 3mm for all foils
    basis_z = basis_x.cross(basis_y).normalise()
    centre_point = centre_point - basis_z * 0.0032

    foil = BolometerFoil(foil_id, centre_point, basis_x, dx, basis_y, dy, slit_objects[slit_id])

    kb5horizontal.add_foil_detector(foil)

kb5horizontal.save('kb5h_camera.json')
